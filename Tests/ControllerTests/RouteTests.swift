...
class RouteTests: XCTestCase {
    ...
    func testNames(){
        ...
        URLRequest(forTestWithMethod: "POST", route: "test2", body: "demon&1".data(using: .utf8))?
            .sendForTestingWithKitura { data in
                if let json = (try? JSONSerialization.jsonObject(with: data, options: .mutableContainers)) as? [String] {
                    print(json)
                    XCTAssertTrue(true, "success")
                } else {
                    //XCTFail("hello POST was nil!")
                }
                printExpectation.fulfill()
        }
//        DBManager.getDocs(database: "donator", handler: {
//            data in
//            print(data!)
//        })
        
        //DBManager.wtf()
        //CloudantConnection.start()
        let data = "[asd, asd2]"
//        if let array = try? JSONSerialization.jsonObject(with: data.data(using: .utf8)!, options: .mutableContainers) as? [String]{
//            print(array)
//        }
        
        //print(String(arc4random_uniform(999999999)) + String(arc4random_uniform(999999999)))
        //print(DBManager.execute("SELECT * FROM ibmx_9c7dd3225c8f81b.donators;"))
